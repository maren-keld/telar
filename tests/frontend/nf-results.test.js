...
test('renderResults preserves result formatting and escapes user content', () => {
  const html = renderResults(
    {
      ...
      calm_pct: 54.56,
      attentive_pct: 45.44,
      baseline_calm_pct: 40,
      baseline_attentive_pct: 35,
      delta_calm_pct: 14.56,
      delta_attentive_pct: 10.44,
      post_series: [],
      spectral: {},
    },
    {
  ...
  assert.match(html, /data-pct="54\.6%" data-time="01:05"/);
  assert.match(html, /data-pct="45\.4%" data-time="00:09"/);
  assert.match(html, /<span>Cambio \(Δ\)<\/span><span>\+14\.6 pp<\/span>/);
  assert.match(html, /<span>Duración de sesión<\/span><span>01:14<\/span>/);
  assert.match(html, /<span>Protocolo<\/span><span>Calma<\/span>/);
