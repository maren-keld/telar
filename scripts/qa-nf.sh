...
if grep_file src/lib/nf-session.js "sessionPhase = 'baseline'" \
  && grep_file src/lib/nf-session.js 'startTraining' \
  && grep_file src/js/modules/neurofeedback.js 'nf-start-training'; then
