...
export function parseAnamnesisJson(raw) {
  ...
  try {
    ...
    if (!ANAMNESIS_REORDER_KEYS.every((key) => key in obj)) return null;
    const parsed = Object.fromEntries(
      ANAMNESIS_REORDER_KEYS.map((key) => [key, String(obj[key] || '').trim()]),
    );
...
export function anamnesisReorderContextText(fields = {}) {
  return ANAMNESIS_REORDER_FIELDS
...
- Devuelve SOLO un JSON con las claves ${jsonKeys}. Sin markdown.`;
  ...
  if (!parsed) throw new Error('La IA no devolvió todos los campos de anamnesis.');
  return parsed;
}
...
export async function renderMotivoConsulta(host, moduleRow) {
  ...
  host.addEventListener('click', async (e) => {
    ...
    try {
      ...
      for (const key of ANAMNESIS_REORDER_KEYS) setVal(key, next[key]);
      await persist();
