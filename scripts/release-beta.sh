#!/usr/bin/env bash
# Release app comercial desde dev-full: tag → CI Mac + Windows.
set -euo pipefail
...
if [[ "$BRANCH" != "dev-full" ]]; then
  echo "Error: ejecuta desde la rama dev-full (app comercial completa). Actual: $BRANCH"
  exit 1
fi
...
echo "→ Versión: $VERSION ($TAG) · rama dev-full"
...
echo "→ Push dev-full…"
git push -u origin dev-full

NOTES="$(cat <<EOF
## Telar ${TAG}

App completa con packs clínicos, neurofeedback Muse 2, IA Mistral y planes Free/Pro.
