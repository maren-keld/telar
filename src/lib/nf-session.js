...
import { getNfWarmupMs, getNfBaselineSec } from './nf-config.js';
...
export class NeurofeedbackSession {
  ...
  updateNeurofeedback(bars) {
    ...
    const training = inTrainingRecording && !warming;
    const { level, percent } = computeFeedbackMetrics(
      this.protocol,
      this.smoothedBars,
      this.feedbackEma,
      { updateEma: (livePreview || inBaseline || inTrainingRecording) && !artifact },
    );

    if ((training || livePreview || inBaseline) && !artifact) {
    ...
    const outLevel =
      warming && !inBaseline
        ...
        : !connected
          ? 0.38
          : artifact && (training || livePreview || inBaseline)
            ? this._lastGoodLevel
            : level;

    const signalQuality = this._updateSignalQuality(artifact);
    const showPct = (livePreview || inBaseline || inTrainingRecording) && !artifact ? percent : null;
    ...
    if ((livePreview || inTrainingRecording) && !inBaseline) {
  ...
  startTraining({ skipped = false } = {}) {
    ...
    if (!this.recording || this.sessionPhase !== 'training') return;
    this._pushMarker('baseline_end');
    this._trainingStartedAt = Date.now();
    this._warmupStartedAt = Date.now();
    if (skipped) this.feedbackEma.reset();
    stopAudioFeedback();
  }
